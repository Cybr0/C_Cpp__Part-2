...
void person_age(Man *array,int size){
	int nowayear =2019;
	for(int i=0; i<size; i++){
		array[i].date.age = (nowayear - array[i].date.year);
	}
}
template <typename T>
...
void printArray(Man *arr, int &size){
	for(int i = 0; i < size; i++){
		...
		<<" "<<arr[i].date.day<<"."
		<<arr[i].date.month<<"."<<arr[i].date.year <<" "<< arr[i].date.age <<" years"<< endl;
	}	
	system("pause");
}
...
void birthday_print(Man *arr, int size, int month){
	for(int i=0; i<size; i++){
		...
		cout<<"Именинники: "<< arr[i].surname<<" "<<arr[i].name<<" "
		<<" "<<arr[i].date.day<<"."
		<<arr[i].date.month<<"."<<arr[i].date.year <<" "<< arr[i].date.age <<" years"<< endl;
		}
	}
