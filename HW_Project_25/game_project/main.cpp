...
pause_menu_state pause_menu(){
	stop_time();
	system("cls");
	float play_time = (get_total_time().count()/1000);
	int p = play_time;
	int hour = p/3600;
	int min = (p % 3600)/60;
...
main_game_state endgame_menu(){
	...
	char enter_player_name[50];
	system("cls");
	std::cout << "CONGRATULATIONS\nYou are WIN!!\n";
	std::cout << "Your time is " << hour << ":" << min << ":" << sec <<std::endl;
	std::cout << "\nEnter your name:\n>";
	std::cin >>  enter_player_name;
	
	add_player(enter_player_name, p);
	check_file();
	selectSort_player();
	system("cls");
	show_statistic();
